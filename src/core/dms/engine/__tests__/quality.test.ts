...
import { classifyQuality, eyeReliable, nearEye } from '../quality';
...
describe('HEAD_ONLY', () => {
  ...
  test('glare on one eye keeps TRACKING; both eyes out → HEAD_ONLY (C-22)', () => {
    expect(q({ tMs: 0, eyeR: { sat: 0.4 } })).toMatchObject({ quality: 'tracking', reliableR: false, reliableL: true });
    expect(q({ tMs: 0, eyeR: { sat: 0.4 }, eyeL: { sat: 0.4 } })).toMatchObject({ quality: 'head_only', reasons: ['eyes_unreliable'] });
    expect(q({ tMs: 0, eyeR: null, eyeL: { irisIn: false } })).toMatchObject({ quality: 'head_only', reasons: ['eyes_unreliable'] });
  });
});

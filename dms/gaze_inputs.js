...
function aspectCorrected(landmarks, width, height) {
  const p = Float64Array.from(check(landmarks));
...
function weak3dCloud(landmarks, width, height) {
  const p = aspectCorrected(landmarks, width, height);
  const r = OUTER_EYE_CORNERS[0], l = OUTER_EYE_CORNERS[1];
  const cx = 0.5 * (p[r * 3] + p[l * 3]);
  const cy = 0.5 * (p[r * 3 + 1] + p[l * 3 + 1]);
...
function eyeCenterAndIod(landmarks, width, height) {
  const p = aspectCorrected(landmarks, width, height);
  const r = OUTER_EYE_CORNERS[0], l = OUTER_EYE_CORNERS[1];
  const center = [0.5 * (p[r * 3] + p[l * 3]), 0.5 * (p[r * 3 + 1] + p[l * 3 + 1])];
  const iod = norm2(p[r * 3] - p[l * 3], p[r * 3 + 1] - p[l * 3 + 1]);
  return { center, iod };
}

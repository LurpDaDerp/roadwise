...
Deno.test('one signed CalculateRoutes call: geo-routes v2, Car, SpeedLimit, simple geometry', async () => {
  ...
  const got = await client.speedLimitAlong(ORIGIN, DEST);
...
Deno.test('a non-2xx answer throws with the status only', async () => {
  ...
  const client = createAwsRoutesClient(env(), { fetch: cap.fetch })!;
  const err = await assertRejects(() => client.speedLimitAlong(ORIGIN, DEST), AwsRoutesError);
...
Deno.test('a transport failure and a timeout throw AwsRoutesError', async () => {
  ...
  const e1 = await assertRejects(() => down.speedLimitAlong(ORIGIN, DEST), AwsRoutesError);
  ...
  const e2 = await assertRejects(() => slow.speedLimitAlong(ORIGIN, DEST), AwsRoutesError);
...
Deno.test('a body that is not JSON throws', async () => {
  ...
  assertInstanceOf(await client.speedLimitAlong(ORIGIN, DEST).catch((e) => e), AwsRoutesError);
});
...
Deno.test('no route, no first-span limit, an unlimited road or a limit off the tables is null', () => {
  assertEquals(readSpeedLimit({ Routes: [] }), null);
  assertEquals(readSpeedLimit({}), null);
  assertEquals(readSpeedLimit(routeBody([])), null);
  assertEquals(readSpeedLimit(routeBody([{ GeometryOffset: 0 }])), null);
  assertEquals(readSpeedLimit(routeBody([{ GeometryOffset: 0, SpeedLimit: { Unlimited: true } }])), null);
  assertEquals(readSpeedLimit(routeBody([{ GeometryOffset: 0, SpeedLimit: { MaxSpeed: 200 } }])), null);
  // only the first span counts: a later span's limit does not stand in for a missing first one
  assertEquals(readSpeedLimit(routeBody([{ GeometryOffset: 0 }, { GeometryOffset: 1, SpeedLimit: { MaxSpeed: 50 } }])), null);
});

Deno.test('the leg is cut to the first span, where AWS says the next one starts', () => {
  const got = readSpeedLimit(routeBody([
    { GeometryOffset: 0, SpeedLimit: { MaxSpeed: 40 } },
    { GeometryOffset: 1, SpeedLimit: { MaxSpeed: 80 } },
  ]));
  assertEquals(got, { mph: 25, leg: [ORIGIN, { lat: 47.6067, lng: -122.32 }] });
});

Deno.test('the leg drops repeated points, needs two, and refuses a position off the globe', () => {
  assertEquals(legForFirstSpan([[-122.32, 47.606], [-122.32, 47.606], [-122.32, 47.607]], null)?.length, 2);
  assertEquals(legForFirstSpan([[-122.32, 47.606], [-122.32, 47.606]], null), null);
  assertEquals(legForFirstSpan([[-122.32, 47.606]], null), null);
  assertEquals(legForFirstSpan([[-122.32, 47.606], [-190, 47.607]], null), null);
  assertEquals(legForFirstSpan([[-122.32, 47.606], ['x', 47.607]], null), null);
  assertEquals(legForFirstSpan('LINESTRING', null), null);
});
...
Deno.test('the leg is kept within put_limits_cache bounds: 1000 positions, under 5 km', () => {
  ...
  assertEquals(legForFirstSpan(many, null)?.length, MAX_LEG_POINTS);

  const long = Array.from({ length: 12 }, (_, i) => [-122.32, 47.6 + i * 0.005]); // ~557 m apart
  const leg = legForFirstSpan(long, null)!;
  let m = 0;

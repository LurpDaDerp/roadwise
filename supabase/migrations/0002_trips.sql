...
--     statement, and act on the user id the edge function derived from the caller's JWT (the
--     `p_user` argument); every row they touch is checked against that id.
...
--     `tracePath` field the device sent.
...
create table public.trips (
  ...
  client_trip_id text not null check (char_length(client_trip_id) between 1 and 64),
...
create table public.score_daily (
  ...
  driving_s int not null default 0 check (driving_s >= 0),
  trips_scored int not null default 0 check (trips_scored >= 0),
  severe_events int not null default 0 check (severe_events >= 0),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
...
-- per-user counters the writers lock to serialise an allowance check with its consumption
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'apply_trip requires the service role' using errcode = 'insufficient_privilege';
  end if;
  ...
  perform public.require_keys('apply_trip', v_payload, 'payload.', array['clientTripId', 'startedAt', 'endedAt', 'tz',
    'distanceM', 'durationS', 'role', 'roleConfidence', 'roleSource', 'mode', 'cameraSession', 'events',
  ...
  perform public.require_type('apply_trip', v_scored->'categoryDeductions', 'scored.categoryDeductions', 'object');
  ...
  v_status := v_scored->>'status';
  if v_status is null or v_status not in ('provisional', 'final', 'unscored', 'discarded') then
    raise exception 'apply_trip status is not a trip status' using errcode = 'invalid_parameter_value';
  end if;
  v_scored_now := v_status in ('provisional', 'final');
  if jsonb_typeof(v_scored->'score') <> 'null' then
    v_score_num := (v_scored->>'score')::numeric;
    if v_score_num < 0 or v_score_num > 100 then
      raise exception 'apply_trip score must be between 0 and 100' using errcode = 'invalid_parameter_value';
    end if;
    v_score := v_score_num::int;
  end if;
  if (v_score is not null) <> v_scored_now then
    raise exception 'apply_trip score must be present exactly when the trip is scored' using errcode = 'invalid_parameter_value';
  end if;
  begin
  ...
  -- the trip's own day row must be among the day rows the caller computed
  if not exists (
  ...
  insert into public.trips (user_id, client_trip_id, started_at, ended_at, local_day, tz, distance_m, duration_s,
    role, role_confidence, role_source, mode, camera_session, score, scoring_version, category_deductions, exposure,
    data_quality, conditions, had_severe_event, limit_coverage_pct, start_geohash5, end_geohash5, polyline, status,
    unscored_reason, trace_path, incomplete)
  values (v_user, v_client, v_started,
    timestamptz 'epoch' + (v_payload->>'endedAt')::bigint * interval '1 millisecond',
    v_local_day, v_payload->>'tz', (v_payload->>'distanceM')::numeric, (v_payload->>'durationS')::numeric,
    ...
    (v_payload->>'hadSevereEvent')::boolean, (p->>'limitCoveragePct')::numeric,
...
-- TypeScript and writes exactly these values. p_scored null skips the trip and its events and
-- refreshes only the day rows and baselines of the trip's owner (the post-delete case);
-- p_events rows are { id (trip_events.id), status, deduction }.
-- ---------------------------------------------------------------------------
create or replace function public.apply_recompute(p_trip_id uuid, p_scored jsonb, p_events jsonb, p_day jsonb, p_baselines jsonb) returns jsonb
language plpgsql security definer set search_path = public as $$
declare
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'apply_recompute requires the service role' using errcode = 'insufficient_privilege';
  end if;
  ...
  if not v_rescore then
    ...
    perform public.require_keys('apply_recompute', p_scored, 'scored.', array['score', 'status', 'categoryDeductions']);
    perform public.require_type('apply_recompute', p_scored->'categoryDeductions', 'scored.categoryDeductions', 'object');
    v_status := p_scored->>'status';
    if v_status is null or v_status not in ('provisional', 'final', 'unscored', 'discarded') then
      raise exception 'apply_recompute status is not a trip status' using errcode = 'invalid_parameter_value';
    end if;
    v_scored_now := v_status in ('provisional', 'final');
    if jsonb_typeof(p_scored->'score') <> 'null' then
      v_score_num := (p_scored->>'score')::numeric;
      if v_score_num < 0 or v_score_num > 100 then
        raise exception 'apply_recompute score must be between 0 and 100' using errcode = 'invalid_parameter_value';
      end if;
      v_score := v_score_num::int;
    end if;
    if (v_score is not null) <> v_scored_now then
      raise exception 'apply_recompute score must be present exactly when the trip is scored' using errcode = 'invalid_parameter_value';
    end if;
  ...
  if not found then
    raise exception 'apply_recompute trip not found' using errcode = 'invalid_parameter_value';
  end if;
  ...
  perform public.upsert_score_day(v_trip.user_id, p_day);
  perform public.upsert_baselines(v_trip.user_id, p_baselines);

  return jsonb_build_object('trip_id', p_trip_id, 'score', v_score, 'status', v_status);
end $$;

-- ---------------------------------------------------------------------------
-- disputes (§9.9): up to 3 consuming disputes per 7 days and at most 20 % of the user's scored
-- events over 30 days may be auto-accepted; "wrong limit" with a stated limit is accepted free.
-- Every dispute is recorded whether or not it is applied. The 7-day counter lives in rate_limits
-- (window anchored at the first consuming dispute, reset once it is 7 days old); the 30-day rule
-- counts event_disputes against trip_events.
-- ---------------------------------------------------------------------------
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'count_dispute_allowance requires the service role' using errcode = 'insufficient_privilege';
  end if;
  if p_user is null then
    raise exception 'count_dispute_allowance requires a user' using errcode = 'invalid_parameter_value';
  end if;

  select count, window_start into v_rl from public.rate_limits where user_id = p_user and key = 'dispute_7d';
  v_used := case when v_rl.count is null or v_rl.window_start < now() - interval '7 days' then 0 else v_rl.count end;
  select count(*)::int into v_disputed from public.event_disputes d
    where d.user_id = p_user and d.auto_accepted and d.created_at >= now() - interval '30 days'
      and not (d.reason = 'wrong_limit' and d.stated_limit_mph is not null);
  ...
  return jsonb_build_object(
    ...
    'can_auto_accept', v_used < v_limit_7d and v_disputed < v_max);
end $$;
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'record_dispute requires the service role' using errcode = 'insufficient_privilege';
  end if;
  if p_user is null then
  ...
  select * into v_trip from public.trips where id = v_ev.trip_id and user_id = p_user and deleted_at is null for update;
  if not found then
    raise exception 'event not owned by user' using errcode = 'insufficient_privilege';
  end if;
  ...
  if exists (select 1 from public.event_disputes where event_id = p_event_id) then
    raise exception 'event already disputed' using errcode = 'insufficient_privilege';
  end if;
  ...
  v_consumes := not (p_reason = 'wrong_limit' and p_stated_limit_mph is not null);

  -- the user's counter row is locked so a concurrent dispute cannot check the allowance before this one consumes it
  ...
  if not v_consumes then
    v_accept := true;
  elsif (v_allow->>'used_7d')::int >= (v_allow->>'limit_7d')::int then
    v_accept := false;
    v_denied := 'allowance_7d';
  elsif not (v_allow->>'can_auto_accept')::boolean then
    v_accept := false;
    v_denied := 'allowance_30d';
  else
    v_accept := true;
  end if;

  insert into public.event_disputes (event_id, user_id, reason, note, stated_limit_mph, auto_accepted)
  values (p_event_id, p_user, p_reason, p_note, p_stated_limit_mph, v_accept)
  ...
  if v_accept then
    ...
    if v_consumes then
      update public.rate_limits
        set count = case when count = 0 or window_start < now() - interval '7 days' then 1 else count + 1 end,
            window_start = case when count = 0 or window_start < now() - interval '7 days' then now() else window_start end
        where user_id = p_user and key = 'dispute_7d';
    end if;
  else
    v_new_status := v_ev.status;
  end if;

  -- wrong-limit reports aggregate per ~150 m geohash cell whether or not the dispute was applied
  if p_reason = 'wrong_limit' and v_ev.lat is not null and v_ev.lng is not null then
  ...
  v_allow := public.count_dispute_allowance(p_user);
  return jsonb_build_object('dispute_id', v_id, 'auto_accepted', v_accept, 'consumed', v_accept and v_consumes,
    'denied_reason', v_denied, 'remaining_7d', v_allow->'remaining_7d', 'remaining_30d', v_allow->'remaining_30d',
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'set_trip_role_row requires the service role' using errcode = 'insufficient_privilege';
  end if;
  if p_user is null then
  ...
  if p_role = 'driver' then
    ...
    update public.trips
      set role = p_role, role_source = 'manual', role_confidence = null,
          status = 'unscored', score = null, unscored_reason = 'passenger'
      where id = p_trip_id;
    update public.trip_events set deduction = null where trip_id = p_trip_id and deduction is not null;
  end if;
...
-- The trace object is removed through the Storage API by the edge function (storage refuses
-- direct row deletes and a row delete would orphan the blob): the key that was stored is returned
-- for that call, and the column is cleared here.
-- ---------------------------------------------------------------------------
...
begin
  if auth.role() is distinct from 'service_role' then
    raise exception 'soft_delete_trip requires the service role' using errcode = 'insufficient_privilege';
  end if;
  if p_user is null then
  ...
  return jsonb_build_object('trip_id', p_trip_id, 'trace_path', v_trip.trace_path);
end $$;
...
revoke all on function public.apply_recompute(uuid, jsonb, jsonb, jsonb, jsonb) from public, anon, authenticated;
...
grant execute on function public.apply_recompute(uuid, jsonb, jsonb, jsonb, jsonb) to service_role;
...
-- ---------------------------------------------------------------------------
-- RLS: default deny; owner-only reads; deleted trips (and their events) hidden from the owner
-- ---------------------------------------------------------------------------

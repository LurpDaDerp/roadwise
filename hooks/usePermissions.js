...
// AVCaptureDevice.requestAccess on iOS, the CAMERA runtime permission on
// Android).  When that module is not linked (Expo Go), expo-image-picker's
// camera permission API is the fallback - it maps to the same OS permission.
// The returned shape stays ('granted' | 'denied' | 'undetermined' | 'unavailable').
...
const cameraViaModule = () => {
  try {
    return !!DmsVision && DmsVision.isAvailable();
  } catch {
    return false;
  }
};
